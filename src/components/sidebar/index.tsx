    ...
    return (
        <nav className="flex flex-col relative h-screen max-w-sm w-full">
            <div className='flex justify-center m-5'>
                <div className="text-white text-lg">PhptoGram</div>
            </div>
            {navItems.map((item) => (
                <div className={cn(buttonVariants({ variant: "default" }),
                    ...
                    <Link to={item.link} className='flex'>
                        <span ><img src={item.icon} alt={item.name} style={{
                            filter: `${pathname === item.link ? "invert(0)" : "invert(1)"}`
                        }} className='w-5 mr-2' /></span>
                        <span>{item.name}</span>
                    </Link>
                </div>
            ))}
            <div className={cn(buttonVariants({ variant: "default" }),
                ...
                <Link to='/login' className='flex' onClick={logOut}>
                    <span ><img src={logoutIcon} alt="Logout" style={{
