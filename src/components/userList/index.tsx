        ...
        return suggestedUser.map((user) => {
            return (
                <div className="flex flex-row items-center mb-4 border-gray-400 justify-start">
                    <span className="mr-2">
                        <img src={user.photoURL ? user.photoURL : avatar} alt="suggestUsers" className='w-10 h-10 rounded-full border-2 border-slate-800 object-cover' />
                    </span>
                    <span className="text-xs">{user?.displayName ? user.displayName : "Guest_user"}</span>
                    <Button className='text-xs p-3 py-2 h-6 bg-slate-700 last-of-type:ml-auto' > Follow</Button>
                </div>
            )
        })
        ...
        <div className="text-white px-8 py-3">
            <Link to="/profile">
                <div className='flex flex-row items-center border-b pb-4 mb-4 border-gray-400 cursor-pointer'>
                    <span className="mr-2">
                        <img src={user?.photoURL ? user.photoURL : avatar} alt="SuggestedUser" className='w-10 h-10 rounded-full border-2 border-slate-800 object-cover' />
                    </span>
                    <span className="text-xs">{user?.displayName ? user.displayName : "Guest_user"}</span>
